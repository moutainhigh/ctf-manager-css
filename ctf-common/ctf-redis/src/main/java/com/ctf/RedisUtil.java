...
public class RedisUtil {
    ...
    public static  boolean lock(String lockKey,long time){

        Long start = System.currentTimeMillis()/1000;
        String uuid = UUID.randomUUID().toString();
        while (true) {
            if (staticRedisTemplate.opsForValue().setIfAbsent(lockKey, uuid)) {
                staticRedisTemplate.expire(lockKey, time, TimeUnit.SECONDS);
                return true;
            }
            if(System.currentTimeMillis()/1000-start>time){
                return false;
            }
        }
    }
    ...
    public static boolean setIfAbsent(String name,String value){
        return staticRedisTemplate.opsForValue().setIfAbsent(name, value);
    }

    public static boolean setIfAbsent(String name, String value, long time){
        boolean flag =setIfAbsent(name, value);
        if(flag){
            staticRedisTemplate.expire(name,time, TimeUnit.SECONDS);
        }
        return flag;
    }

    public static int incr(String key){
        Long increment = staticRedisTemplate.opsForValue().increment(key);
        return increment.intValue();
    }

    public static boolean unLock(String lockKey){
        return staticRedisTemplate.delete(lockKey);
    }
}
